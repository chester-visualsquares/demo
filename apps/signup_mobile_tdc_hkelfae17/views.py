...
from signup_mobile_tdc_hkelfae17.constants import ENGLISH, SIMPLIFIED_CHINESE
from signup_mobile_tdc_hkelfae17.forms import SignupForm, WechatSignupForm
from signup_mobile_tdc_hkelfae17.models import Contact, SignupCode
...
@signed_request_required(settings.SNM_TDC_HKELFAE17_FB_APP_SECRET)
@language_enabled
def canvas_landing(
        request,
        template_name='signup_mobile_tdc_hkelfae17/canvas_landing.html'):
    ...
    if aid:
        ...
        query_parm = (('app_data', json.dumps(app_data)),) + \
            settings.SNM_TDC_HKELFAE17_FB_PAGE_PARAM
    else:
        query_parm = settings.SNM_TDC_HKELFAE17_FB_PAGE_PARAM

    query_string = urllib.urlencode(query_parm)
    fb_page_url = '%s?%s' % (settings.SNM_TDC_HKELFAE17_FB_PAGE_URL,
                             query_string)
...
def landing(request):
    ...
    return _redirect('snm_tdc_hkelfae17_info', app_data)


@require_http_methods(['GET'])
@language_enabled
def info(request, mobile=False,
         template_name_desktop='signup_mobile_tdc_hkelfae17/info.html',
         template_name_mobile='signup_mobile_tdc_hkelfae17/info_mobile.html'):
    template_name = template_name_mobile if mobile else template_name_desktop
    next_url = (reverse('snm_tdc_hkelfae17_signup_mobile') if mobile
                else reverse('snm_tdc_hkelfae17_signup'))
    aid = request.GET.get(settings.CS_TRACKING_PARAMETER, None)
...
def info_website(
        ...
        template_name_website='signup_mobile_tdc_hkelfae17/info.html',
        template_name_wechat='signup_mobile_tdc_hkelfae17/info_wechat.html',
        template_name_mobile='signup_mobile_tdc_hkelfae17/info_mobile.html'):
    if network == 'website':
        template_name = (template_name_mobile
                         if mobile else template_name_website)
        next_url = reverse('snm_tdc_hkelfae17_signup_website')
    elif network == 'wechat':
        template_name = template_name_wechat
        next_url = reverse('snm_tdc_hkelfae17_signup_wechat')
    ...
    if request.method == 'GET':
        ...
        if not aid:
            aid_cookie = [value for key, value in request.COOKIES.items()
                          if '.'.join(('_pk_id',
                                       settings.SNM_TDC_HKELFAE17_CS_ACCOUNT_ID
                                       )) in key]
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                if contact.attend_elec:
                    code_elec = SignupCode.objects.filter(
                        used=False, is_elec=True)[0]
                    code_elec.used = True
                    code_elec.save()
                    contact.signup_code_elec = code_elec.value
                if contact.attend_light:
                    code_light = SignupCode.objects.filter(
                        used=False, is_light=True)[0]
                ...
                if fb_psid != '':
                    ...
                    data_ref = ('signup_mobile_tdc_hkelfae17.Contact|%s'
                                % contact.auth_str)
                    create_mgr_signups(
                        settings.SNM_TDC_HKELFAE17_FB_PAGE_ID,
                        fb_psid, data_ref)
...
def signup_website(
        ...
        template_name_website='signup_mobile_tdc_hkelfae17/signup.html',
        template_name_wechat='signup_mobile_tdc_hkelfae17/signup_wechat.html',
        template_name_mobile='signup_mobile_tdc_hkelfae17/signup_mobile.html'):
    if network == 'website':
        template_name = (template_name_mobile
                         if mobile else template_name_website)
        next_view = 'snm_tdc_hkelfae17_thanks_website'
    elif network == 'wechat':
        template_name = template_name_wechat
        form_cls = WechatSignupForm
        next_view = 'snm_tdc_hkelfae17_thanks_wechat'
        request.language = SIMPLIFIED_CHINESE
...
def thanks(
        ...
        template_name_desktop='signup_mobile_tdc_hkelfae17/thanks.html',
        template_name_mobile='signup_mobile_tdc_hkelfae17/thanks_mobile.html'):
    template_name = template_name_mobile if mobile else template_name_desktop
    ...
    if auth_str:
        pass_through_param = base64.urlsafe_b64encode(
            'signup_mobile_tdc_hkelfae17.Contact|' + auth_str)
    else:
...
def thanks_website(
        ...
        template_name_website='signup_mobile_tdc_hkelfae17/thanks.html',
        template_name_wechat='signup_mobile_tdc_hkelfae17/thanks_wechat.html',
        template_name_mobile='signup_mobile_tdc_hkelfae17/thanks_mobile.html'):
    if network == 'website':
        template_name = (template_name_mobile
                         if mobile else template_name_website)
